...
cars_meeting_criteria = { key: values for key, values in car_data.items() # creating a dictionary of cars that meet the specified search criteria
                         if values[1] >= search_criteria[0]
                         and values[2] >= search_criteria[1]
                         and values[4] <= search_criteria[2]
                         }
sorted_cars = sorted(cars_meeting_criteria.items(), key=lambda item: item[1][4]) # sorting the cars by price in ascending order
print("Cars matching the search criteria:")
count_cars = 0
for key, values in sorted_cars:
  print(f"{key}:{values}")
  count_cars += 1
  if count_cars == 5: # displaying only the first five cars
    break
